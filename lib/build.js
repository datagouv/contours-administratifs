...
async function writeGeoJSON(features, interval, destPath) {
...
async function buildAndWriteEPCI(simplifiedCommunes, interval) {
  ...
  await writeGeoJSON(epci, interval, join(destPath, `epci-${interval}m.geojson`))
}
...
async function buildAndWriteDepartements(simplifiedCommunes, interval) {
  ...
  await writeGeoJSON(departements, interval, join(destPath, `departements-${interval}m.geojson`))
}
...
async function buildAndWriteRegions(simplifiedCommunes, interval) {
  ...
  await writeGeoJSON(regions, interval, join(destPath, `regions-${interval}m.geojson`))
}
...
async function buildAndWriteCommunes(simplifiedCommunes, interval) {
  ...
  await writeGeoJSON(communes, interval, join(destPath, `communes-${interval}m.geojson`))
}
...
async function buildAndWriteArrondissements(simplifiedArrondissements, interval) {
  ...
  await writeGeoJSON(arrondissements, interval, join(destPath, `arrondissements-municipaux-${interval}m.geojson`))
}
